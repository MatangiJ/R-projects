cran <- getOption("repos")      #connects to local repository

#using deep learning s/w frame work of apache mxnet.
...
library(DiagrammeR)  #builds graphs/network structures
library(mxnet)


# Directory for images
image_dir <- "D:/Others/Downloads/dogs-vs-cats/train/train"
source("https://bioconductor.org/biocLite.R")
biocLite("EBImage")    #provides general purpose functionality for image processing & analysis
library(EBImage)

#construct path to file & display the image
example_cat_image <- readImage(file.path(image_dir, "cat.0.jpg",package="EBImage"))
...
#images in the dataset are of various sizes,
...
extract_feature <- function(dir_path, width, height, is_cat = TRUE, add_label = TRUE) {
  img_size <- width*height     #did not understand this code!

## List images in path
images_names <- list.files(dir_path)
  if (add_label) {
## Select only cats or dogs images
## grep() --> searches for a particular pattern in each element
    ## grepl() --> returns TRUE when a particular pattern is found    
...
## Set label, cat = 0, dog = 1
  ...
  feature_list <- pblapply(images_names, function(imgname) {
    ...
    ## Set to grayscale
    grayimg <- channel(img_resized, "gray")    #channel -->extracts a specific element from the object, here, grayscale
...
#process cats and dogs images separately
...
dim(cats_data)                          #saving into a data frame
dim(dogs_data)
...
training_index <- createDataPartition(complete_set$label, p = .7, times = 1)
...
sum(diag(table(test_data[, 1], predicted_labels)))/7500
